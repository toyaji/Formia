...
- **저장**: OS 네이티브 보안 저장소 (macOS Keychain 등)는 **로컬 설정** 및 **오프라인 캐시** 용도로만 사용.
- **호출**: 모든 AI 호출은 **서버 프록시**를 경유하여 수행 (Web과 동일).
- **동기화**: 로그인 상태에서 서버와 항상 동기화.
- **보안 수준**: ★★★★☆ (서버 프록시 보안 + OS 보안 저장소)
...
│  보안 수준 비교                                                   │
│                                                                  │
│  Tauri (Desktop) / Web (Browser) 공통 전략                        │
│  ──────────────────────────────────────────                      │
│  키 저장: 서버 DB (암호화) + Desktop 로컬 캐시(선택적)            │
│  AI 호출: 서버 프록시 (/api/ai/generate)                         │
│  서버 경유: ✅ 필수 (보안 및 관리 일원화)                         │
│  오프라인: ❌ 불가능 (AI 서비스 특성)                            │
│                                                                  │
│  보안: ★★★★☆                                                   │
│  편의: ★★★★★                                                   │
│                                                                  │
│  권장: 통합 프록시 모델 (관리 효율 및 일관성)                     │
└──────────────────────────────────────────────────────────────────┘
```
...
| **공용 PC 데이터 노출**           | 이전 사용자의 작업 내용이 다음 사용자에게 노출    | 게스트 모드에서 자동 복구 금지, 명시적 복구 프롬프트 도입.    |

### 8.3 Shared Public PC Data Protection (Guest Mode)

Formia는 로그인 없이도 사용 가능한 '게스트 모드'를 지원하며, 이는 학교, 도서관 등 공용 PC 환경에서도 자주 사용될 수 있습니다. 이를 위한 특화된 보안 정책을 적용합니다.

#### 1. Privacy-First Restoration (명시적 복구)
게스트 사용자의 작업 내용은 브라우저에 저장되지만, **페이지 새로고침 시 자동으로 불러오지 않습니다.** 
- **이유**: 이전 사용자가 탭을 닫고 나간 뒤, 다음 사용자가 페이지를 열었을 때 이전 사용자의 작업 내용이 바로 노출되는 것을 방지하기 위함입니다.
- **메커니즘**: "이어서 작업하시겠습니까?" 팝업을 통해 본인 확인(혹은 의사 확인)을 거친 경우에만 데이터를 로드합니다. "새로 시작" 선택 시 이전 데이터는 보안 삭제됩니다.

#### 2. Ephemeral Chat Context (휘발성 채팅)
AI 에이전트와의 대화에는 기획 의도, 민감한 비즈니스 데이터 등이 포함될 수 있습니다.
- **정책**: 게스트 모드의 채팅 내역은 `localStorage`나 서버 DB에 영구 저장되지 않습니다.
- **보호**: 페이지 새로고침이나 로그아웃 시 채팅 내역은 즉시 파기되어, 다음 사용자나 제3자가 이전 대화 내용을 엿볼 수 없도록 합니다.
