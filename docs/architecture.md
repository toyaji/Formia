...
### 4.4 Persistence & Session Strategy

Formia는 사용자의 로그인 상태에 따라 다른 지속성 전략을 사용하여 편의성과 보안의 균형을 맞춥니다.

#### 1. 로그인 사용자: Cloud-First Sync
- **즉시 동기화**: 폼 에디터 진입 시 서버에 즉시 영구 ID를 생성하고, 모든 변경 사항을 실시간으로 클라우드에 저장합니다.
- **연속성**: 언제 어디서나 로그인만 하면 작업 중이던 최신 상태로 연결됩니다.

#### 2. 비로그인 사용자 (Guest): Privacy-First Recovery
- **로컬 우선 저장**: 모든 작업은 브라우저의 `localStorage`에 즉시 백업됩니다.
- **복구 프롬프트 (Restoration Prompt)**: 공용 PC나 공유 환경에서 이전 사용자의 작업 내용이 무단 노출되는 것을 방지하기 위해, 새로고침 시 **자동으로 데이터를 불러오지 않습니다.** 
- **명시적 승인**: 사용자가 "이어서 작업하기"를 명시적으로 선택했을 때만 로컬 초안을 불러오며, "새로 시작" 선택 시 이전 데이터는 즉시 파기됩니다.

#### 3. Ephemeral Chat Session
- **세션 범위 채팅**: AI 에이전트와의 채팅 기록은 보안 및 컨텍스트 정리를 위해 브라우저 영구 저장소에 기록되지 않습니다.
- **휘발성**: 페이지 새로고침이나 로그아웃 시 채팅 내역은 초기화되며, 이전 세션의 민감한 정보가 남지 않도록 합니다. (로그인 사용자의 경우 클라우드에 저장된 히스토리를 선택적으로 불러올 수 있습니다.)
